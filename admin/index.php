<?php
 ...
 // include 'includes/trial.php';

  $txnQuery = "
  SELECT t.id, t.cart_id, t.full_name, t.description, t.txn_date, t.grand_total, c.items, c.paid,c.shipped
  FROM transactions t
      ...
      <tbody>
        ...
        ?>
            <tr <?=(date("m") == $i)?' class="info"':'';?>>
              <td><?=$dt->format("F");?></td>
              <td><?=((array_key_exists($i, $last))?money($currentTotal):money(0));?></td>
    ...
    while ($product = mysqli_fetch_assoc($iQuery)) {
      ...
      foreach ($sizes as $size){
        ...
        $item = array(
           ...
           'category' => $cat['parents'].' ~ '.$cat['child']
        );
        $lowItems[] = $item;
      }
      }
