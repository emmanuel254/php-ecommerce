...
if (isset($_POST) && !empty($_POST)) {
	$post_parent= sanitize($_POST['parents']);
	$category = sanitize($_POST['category']);
	$sqlform = "SELECT * FROM categories WHERE category = '$category' AND parents = '$post_parent'";

	if (isset($_GET['edit'])) { 
	$id = $edit_category['id'];
	$sqlform = "SELECT * FROM categories WHERE category = '$category' AND parents = '$post_parent' AND id = '$id'";	
	}
   ...
   #if category already exists...error message
   if ($count > 0) {
   	$errors[] .= $category.' already exists.Please add a different type.';
   }
   ...
   #update database
  	$updatesql = "INSERT INTO categories(category, parents) VALUES ('$category ','$post_parent')";
  	if (isset($_GET['edit'])) {
  		$updatesql = "UPDATE categories SET category = '$category', parents = '$post_parent' WHERE id = '$edit_id'";
...
if (isset($_GET['edit'])) {
	...
	$parent_value = $edit_category['parents'];
}
else{
	if (isset($_POST)) {
