...
<script>
 ...
 function update_info(){
  ...
  jQuery('select[name = "category"]').change(function(){
   ...
   function get_clothe_type(selected){
    if (typeof selected === 'undefined') {
      var selected = '';
    }
    var categoryId = jQuery('#category').val();
    jQuery.ajax({
      url  : '/boutique/admin_panel/parsers/clothe_type.php',
      type : 'post',
      data :  {categoryId : categoryId, selected : selected},
      success : function(data){
        jQuery('#brand').html(data);
  ...
  function user_info(id){
    jQuery('#error').html("");
    var data = jQuery('#admin_form').serialize();
    
    jQuery.ajax({
      url : '/boutique/admin_panel/parsers/update_user.php',
      method : 'post',
      data : data,
