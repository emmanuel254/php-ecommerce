    ...
    <table class="table table-bordered table-striped table-condensed">
      <thead>
      <th style="font-size: 34px;"><?=$branch;?></th>
      <th style="font-size: 34px;">Brands</th>
      <th></th>
      </thead>
      <tbody>
        <?php
        $brand = $db->query("SELECT * FROM accesoriestype WHERE parent = 0 AND branch = '$branch' 
          ORDER BY brand_name");
         $i = 1;
        ?>
        <tr>
        ...
        <tr style="background-color: palevioletred;">
          ...
          <td width="100px">
      <a href="products.php?branch=<?=$branch;?>&delete_brand&id=<?=$brandR['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-remove"></span></a>
        <a href="products.php?branch=<?=$branch;?>&edit_brand&id=<?=$brandR['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-pencil"></span></a>
          </td>
        </tr>
        <?php $i++;?>
          <?php endwhile;?>
    ...
    <a class="btn btn-xs btn-primary" href="products.php?branch=<?=$branch;?>&brand=1">add brand</a>
<!-- Creates a form for adding brand -->
  <?php
     ...
     // echo($brand);
         ...
         if (!empty($errors)) {
          ...
          $sql = ("INSERT INTO accesoriestype (brand_name,parent,branch) VALUES ('$brand',0,'$branch')") or die(mysqli_error($db));
  ...
  <?php if(isset($_GET['brand']) || isset($_GET['edit_brand'])):?>
    <form class="form-inline pull-right" method="POST" action="products.php?branch=<?=$branch;?>&<?=((isset($_GET['edit_brand']))?'edit_brand&id='.$id:'brand=1')?>">
    <?php
          if (isset($_GET['brand'])) {
            $brand_value = '';
          }
    ...
    ?>
      ...
      <a href="products.php?branch=<?=$branch;?>" class="btn btn-warning">Cancel</a>
