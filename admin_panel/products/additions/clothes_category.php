...
<h2 class="text-center">Category & its Brands</h2>
<table class="table table-bordered table-condensed table-striped">
	<thead>
		<th>#</th>
		<th><h3>Brand Types</h3></th>
		<th width="105px"></th>
	</thead>
	<tbody>
	  <?php
         $sql = "SELECT * FROM accesoriestype WHERE parent = 0 AND branch = 44 ORDER BY brand_name";
         $pQuery = $db->query($sql);
         $i = 1;
       ...
       <?php
        ...
        ?>
        	...
        	<td>
        	<a href="clothes.php?delete_category&id=<?=$cresult['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-remove"></span></a>
		    <a href="clothes.php?edit_category&id=<?=$cresult['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-pencil"></span></a>
        	</td>
        </tr>
      <?php endwhile; ?>
	  <?php $i++; endwhile; ?>
...
if ($_POST) {
 ...
 $sql = $db->query("SELECT * FROM accesoriestype WHERE parent = '$brand' AND brand_name = '$category'");	
      ...
      if ($brand == 0) {
      	$errors[] = 'Categories are added in the <i><b>CATEGORY</b></i> section';
      }
      if (!empty($errors)) {
      	...
      	// add to database or edit database
      	  $sql2 = 
      	  ("INSERT INTO accesoriestype(parent,brand_name,branch) VALUES ('$brand','$category',44)");
      	if (isset($_GET['edit_category'])) {
...
<div class="row">
	<div class="col-md-4">
	<form action="clothes.php?<?=((isset($_GET['edit_category']))?'edit_category&id='.$edit_id:'add_category=1');?>" method="post" class="form">
	
		<legend for="brand">Category:</legend>
		<?php 
            $brandQ = $db->query("SELECT * FROM accesoriestype WHERE parent = 0 AND branch = 44");
		?>
...
<div class="col-md-3">
	<a href="clothes.php?add_category=1" style="display: <?=(isset($_GET['edit_category']) || isset($_GET['add_category']))?'none;':'';?>" class="btn btn-xs btn-primary">add category</a>
