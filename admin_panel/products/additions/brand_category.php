...
<table class="t table table-bordered table-condensed table-striped">
	<thead>
		<th>#</th>
	...
	<tbody>
	  <?php
       $sql = "SELECT * FROM accesoriestype WHERE parent = 0 AND branch = '$branch' ORDER BY brand_name";
         $pQuery = $db->query($sql);
         $i = 1;
       ...
       <?php
        ...
        ?>
        	...
        	<td>
        	<a href="products.php?branch=<?=$branch;?>&delete_category&id=<?=$cresult['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-remove"></span></a>
		    <a href="products.php?branch=<?=$branch;?>&edit_category&id=<?=$cresult['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-pencil"></span></a>
        	</td>
        </tr>
      <?php endwhile; ?>
	  <?php $i++; endwhile; ?>
...
if (isset($_GET['delete_category'])) {
	...
	?>
	<script>window.location = 'http://localhost/boutique/admin_panel/products/products.php?branch=<?=$branch;?>';
         </script>
	<?php
	 ...
	 $sql = $db->query("SELECT * FROM accesoriestype WHERE parent = '$brand' AND brand_name = '$category'");	
      ...
      if (!empty($errors)) {
      	...
      	// add to database or edit database
      	  $sql2 = ("INSERT INTO accesoriestype(parent,brand_name,branch) 
      	  	        VALUES ('$brand','$category','$branch')");
      	if (isset($_GET['edit_category'])) {
...
<div class="row">
	<div class="col-md-4">
	<form action="products.php?branch=<?=$branch?>&<?=((isset($_GET['edit_category']))?'edit_category&id='.$edit_id:'add_category=1');?>" method="post" class="form">
	
		<legend for="brand">Brand:</legend>
		<?php 
            $brandQ = $db->query("SELECT * FROM accesoriestype WHERE parent = 0 AND branch = '$branch'");
		?>
   ...
   <a href="products.php?branch=<?=$branch; ?>" class="btn btn-xs btn-warning">Cancel</a>
...
<div class="col-md-3">
	<a href="products.php?branch=<?=$branch;?>&add_category=1" style="display: <?=(isset($_GET['edit_category']) || isset($_GET['add_category']))?'none;':'';?>" class="btn btn-xs btn-primary">add category</a>
