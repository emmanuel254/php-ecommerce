...
//initialization of the values to be entered in the database
  ...
  $parentQ = $db->query("SELECT * FROM accesoriestype WHERE parent = 0 AND branch = 44");
  $saved_image = '';
...
?>
  <div class="row">
  	...
  	<div class="col-md-10">
  		<h2 class="text-center">Add Clothings</h2>
  		<form action="clothes.php?<?=((isset($_GET['edit']))?'edit&id='.$edit_id:'add=1')?>" method="POST" enctype="multipart/form-data">
  			<div class="row">
  				<div class="col-md-3">
        ...
        <?php endif; ?>
            ...
            <div class="col-md-6">
  			   	...
  			   	<a href="clothes.php" style="margin-right: 15px;" class="btn btn-xs btn-warning pull-right">Cancel</a>
           </div>
  			</div>
  		</form>
...
<?php }else{
 $productQ = $db->query("SELECT * FROM accesories WHERE deleted = 0 AND branch = 44");
  $i = 1;
?>
<h1 class="text-center">CLOTHINGS</h1>
<div class="row">
	<div class="col-md-1"></div>
	<div class="col-md-10">
<table class="table table-condensed table-striped table-bordered">
    ...
    <td><?=$result['list_price'];?></td>
		<td></td>
		<td width="200px">
			<a href="clothes.php?featured=<?=(($result['featured']==0)?'1':'0'); ?>&id=<?=$result['id'];?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-<?=(($result['featured'] == 1)?'minus':'plus');?>"></span></a>
    	&nbsp <?=(($result['featured']==1)?'Featured Product':'');?>
       </td>
		<td><?=$result['sold']?></td>
		<td style="width: 150px;">
            <a href="clothes.php?delete&id=<?=$result['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-remove"></span></a>
		    <a href="clothes.php?edit&id=<?=$result['id'];?>" class="btn btn-xs pull-right"><span class="glyphicon glyphicon-pencil"></span></a>
		</td>
		</tr>
		<?php $i++;?>
		<?php endwhile;?>	
   	  ...
   	  <a class="btn btn-xs btn-primary pull-right" href="clothes.php?add=1">Add Clothe</a>
   </div>
   <div class="col-md-1"></div>
</div>
...
<div class="row">
	<div class="col-md-1"></div>
	<div class="col-md-5">
		<?php include 'additions/clothes_available.php'; ?>
     </div>
	<div class="col-md-5">
		<?php include 'additions/clothes_category.php';?>
	</div>
	<div class="col-md-1"></div>
</div>
</div>
